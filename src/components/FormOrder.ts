...
import { IFormFields, IOrderModel, PayType } from "../types";
import { ensureElement } from "../utils/utils";
import {IEvents} from './base/events';
 
 export class FormOrder  extends FormView<Pick<IOrderModel, 'address' | 'payType' >>{
  protected _buttonCash: HTMLButtonElement;
  protected _buttonCard: HTMLButtonElement;
  constructor(protected container: HTMLFormElement, protected events: IEvents) {
    super(container, events);
    this._buttonCard = ensureElement<HTMLButtonElement>('.button_alt[name=card]', container)
    this._buttonCash = ensureElement<HTMLButtonElement>('.button_alt[name=cash]', container)
    this.valid = false;
