...
[ToDo]: <> (Не совсем понятно, что должен содержать класс)

Класс реализует отображениее корзины товаров. Наследует класс Component и реализует класс IBusketView. Является элементом слоя представления. [ToDo]
...
Интерфейс относится к слою представления. Описывает поля ввода для формы.Используется в классе FormView.

# ToDo Здесь что-то не то. Разобраться
