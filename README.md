...
### Класс BusketView
Класс реализует отображениее корзины товаров. Наследует класс Component и реализует класс IBusketView. Является элементом слоя представления. [ToDo] [ToDo_1]

[ToDo_1]: <> (Не совсем понятно, что должен содержать класс)
...
Интерфейс относится к слою представления. Описывает поля ввода для формы.Используется в классе FormView. [ToDo] [ToDo_2]

[ToDo-2]: <> (Так ли должно быть? Интерфейс использую как содержание формы)
